...
当你创建一个转账时，你可以从可用列表中选择一个存钱罐。

如果该存钱罐与转账的目的地账户相关联，则交易金额将被加到该存钱罐里。
如果该存钱罐与转账的源账户相关联，则金额将从该存钱罐转出。
不论情况如何，转账_总是_会被创建。如果该存钱罐已满，则它不会发生变化。在其他情况下，金额会一直添加，直到填满为止。任何剩余的金额仍将继续被转移，但不会加到一个存钱罐中。
